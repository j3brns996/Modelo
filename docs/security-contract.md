...
- Presentation uses local assets and system fonts. The generated CSP forbids
  remote scripts and fonts. Configuration and output tests own this boundary.
- v0.1 stores no credentials, account identifiers, private endpoints, offer
  tokens or confidential evidence bodies. Input validation rejects defined
  credential fields and unsafe URLs; publication tests use leak canaries.
  UBS adds source heuristics with reviewed findings. These checks do not claim
  comprehensive secret detection across all source, logs and artifacts.
