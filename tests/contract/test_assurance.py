...
def test_mapping_rejects_bad_claims_and_missing_artefacts():
    for field, value in [
        ...
        ("artefacts", ["missing.file"]),
        ("source", "https://example.invalid/not-nist"),
    ]:
        document = mapping()
    ...
    item["operating_assessments"] = [
        {
            ...
            "remediation": "none",
            "independent_review_ref": "example",
        }
    ]
    assert any(
        "non-operating" in error
        for error in validate_mapping(ROOT, document, as_of=date(2026, 9, 6))
    )
    item["operating_assessments"][0]["period_end"] = "2099-01-01"
    assert any(
        "elapsed period" in error
        for error in validate_mapping(ROOT, document, as_of=date(2026, 9, 6))
    )
...
def test_contract_mapping_maturity_and_site_do_not_drift():
    ...
    assert profile["current_levels"] == {
        item["id"]: item["current_level"] for item in mapping()["mappings"]
    }
...
def test_diagrams_and_external_examples_are_not_catalogue_authority():
    ...
    assert {yaml.safe_load(example)["kind"] for example in examples} == {
        "AIUseBinding",
        "InventoryCoverageDetermination",
    }
