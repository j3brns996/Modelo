...
class ValidatorTests(unittest.TestCase):
    ...
    def check(
        self, base: str | None = None, head: str | None = None, as_of: date = date(2026, 9, 1)
    ):
    ...
    def install_profile(self, transform=lambda record, offering: None) -> None:
        profile = {
            "source": {
                ...
                "partition": "aws",
                "region": "eu-west-2",
                "sanitised_parameters": {"inferenceProfileIdentifier": "eu.test.profile-v1"},
            ...
            "projection": {
                "profileId": "eu.test.profile-v1",
                "type": "SYSTEM_DEFINED",
                "status": "ACTIVE",
                "models": [
                    {"modelArn": "arn:aws:bedrock:eu-west-2::foundation-model/test.model-v1"}
                ],
            },
            "visibility": "public",
        }
    ...
    def test_unknown_reference_and_path_identity_fail(self) -> None:
        ...
        text = path.read_text(encoding="utf-8").replace(
            "model_id: test-model", "model_id: missing-model"
        )
    ...
    def test_evidence_value_mismatch_and_freshness_fail(self) -> None:
        path = self.repository.root / "catalogue/models/test-model.yaml"
        path.write_text(
            path.read_text(encoding="utf-8").replace("name: Test Model", "name: Other Model"),
            encoding="utf-8",
            newline="\n",
        )
        head = self.repository.commit()
    ...
    def test_evidence_edit_is_both_content_and_change_invalid(self) -> None:
        ...
        path.write_text(
            path.read_text(encoding="utf-8").replace("visibility: public", "visibility: internal"),
            encoding="utf-8",
            newline="\n",
        )
        head = self.repository.commit()
    ...
    def test_condition_version_edit_is_immutable(self) -> None:
        ...
        path.write_text(
            path.read_text(encoding="utf-8").replace("Synthetic condition", "Changed condition"),
            encoding="utf-8",
            newline="\n",
        )
        head = self.repository.commit()
    ...
    def test_scheduled_audit_detects_historical_condition_mutation(self) -> None:
        ...
        path.write_text(
            original.replace("Synthetic condition", "Historically changed condition"),
            encoding="utf-8",
            newline="\n",
        )
        final = self.repository.commit("historical mutation")
        self.assertIn(
            "CHANGE_INVALID", {finding.code for finding in self.check(base=final, head=final)}
        )
    ...
    def test_changed_condition_reintroduction_after_deletion_fails(self) -> None:
        ...
        path.write_text(
            original.replace("Synthetic condition", "Reintroduced meaning"),
            encoding="utf-8",
            newline="\n",
        )
        ...
        self.assertIn(
            "CHANGE_INVALID", {finding.code for finding in self.check(base=deleted, head=head)}
        )
    ...
    def test_direct_route_requires_correlated_aws_bedrock_api_evidence(self) -> None:
        cases = {
            "documentation": lambda record: record.update(
                source={
    ...
    def test_direct_route_arn_must_match_source_partition_and_region(self) -> None:
        ...
        self.assertIn(
            "EVIDENCE_VALUE_MISMATCH", {finding.code for finding in self.check(head=head)}
        )
            ...
            with self.subTest(partition=partition, region=region):
                ...
                try:
                    ...
                    def transform(record, offering):
                        ...
                        offering["evidence_refs"]["/routes/0/reference"]["projection_pointer"] = (
                            "/modelArn"
                        )
    ...
    def test_profile_requires_correlated_api_source_system_status_and_complete_destinations(
        self,
    ) -> None:
        cases = {
            "documentation": lambda profile, offering: profile.update(
                source={
                    "type": "official-provider-documentation",
                    "uri": "https://example.invalid/aws-docs",
                }
            ),
            "wrong-operation": lambda profile, offering: profile["source"].update(
                operation="GetFoundationModel"
            ),
            "wrong-provider": lambda profile, offering: profile["source"].update(
                provider="not-aws"
            ),
            "wrong-service": lambda profile, offering: profile["source"].update(
                service="not-bedrock"
            ),
            "wrong-source-region": lambda profile, offering: profile["source"].update(
                region="eu-west-1"
            ),
            "wrong-partition": lambda profile, offering: profile["source"].update(
                partition="aws-cn"
            ),
            "application-profile": lambda profile, offering: profile["projection"].update(
                type="APPLICATION"
            ),
            "inactive-profile": lambda profile, offering: profile["projection"].update(
                status="INACTIVE"
            ),
    ...
    def test_profile_destination_bijection_is_by_pointer_not_repeated_value(self) -> None:
        def transform(profile, offering):
            profile["projection"]["models"].append(deepcopy(profile["projection"]["models"][0]))
    ...
    def test_profile_destination_arn_must_match_destination_evidence_region(self) -> None:
        ...
        offering = yaml.safe_load(offering_path.read_text(encoding="utf-8"))
        offering["routes"][0]["model_binding"]["destinations"][0]["model_evidence"]["id"] = (
    ...
    def test_mixed_offering_route_mismatch_yields_exactly_one_diagnostic_and_preserves_aws_rigor(
        self,
    ) -> None:
        ...
        offering["routes"].append(self._gcp_shaped_route("gcp-mismatched", mismatched_reference))
        ...
        evidence_path.write_text(
            yaml.safe_dump(availability_evidence, sort_keys=False), encoding="utf-8", newline="\n"
        )
        offering["evidence_refs"]["/routes/1/reference"] = {
            "id": evidence_identifier,
            "projection_pointer": "/reference",
        }
        offering_path.write_text(
            yaml.safe_dump(offering, sort_keys=False), encoding="utf-8", newline="\n"
        )
    ...
    def test_schema_valid_gcp_vertex_adapter_has_no_implemented_validator_and_no_evidence_cascade(
        self,
    ) -> None:
