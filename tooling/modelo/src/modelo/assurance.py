...
def validate_mapping(root: Path, document: Mapping[str, Any], *, as_of: date) -> tuple[str, ...]:
    ...
    errors = [
        finding.message
        for finding in schemas.validate("nist-ai-rmf-mapping.schema.json", document, MAPPING_PATH)
    ]
    if errors:
        return tuple(errors)
    ...
    for item in document["mappings"]:
        expected_source = (
            ...
            if item["id"] == "GOVERN-1.6"
            else "https://nvlpubs.nist.gov/nistpubs/ai/NIST.AI.600-1.pdf"
        )
        if item["source"] != expected_source:
            errors.append("source does not own the selected NIST outcome")
        if item["current_level"] == "O":
            errors.append(
                "this non-operating repository profile cannot assert operating equivalence; a separately governed operating assessment profile is required"
            )
        ...
        for assessment in item["operating_assessments"]:
            start, end = (
                date.fromisoformat(assessment["period_start"]),
                date.fromisoformat(assessment["period_end"]),
            )
