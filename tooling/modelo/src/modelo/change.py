...
def validate_changes(
    ...
    *,
    ...
    offering_additions = [
        path for status, path in changes if status == "A" and path.startswith(f"{offerings_root}/")
    ]
    offering_deletions = [
        path for status, path in changes if status == "D" and path.startswith(f"{offerings_root}/")
    ]
    if (
        offering_additions
        and offering_deletions
        and (len(offering_additions) != 1 or len(offering_deletions) != 1)
    ):
        diagnostics.append(
            Diagnostic(
                "CHANGE_INVALID",
    ...
    for status, path in changes:
        ...
        if (
            status == "D"
            and not path.startswith(f"{offerings_root}/")
            and not (path.startswith(f"{evidence_root}/") or path.startswith(f"{conditions_root}/"))
        ):
            diagnostics.append(
                Diagnostic(
                    "CHANGE_INVALID",
...
def validate_reserved_identity_history(
    root: Path,
    base: str,
    changes: tuple[tuple[str, str], ...],
    models_root: str,
    ...
    additions = [
        ...
        for status, path in changes
        if status == "A"
        for kind_root in (models_root, offerings_root)
        if path.startswith(kind_root + "/")
    ]
    if not additions:
        return ()
    diagnostics = []
    ...
    for commit in _history_commits(root, base):
        for kind_root in reserved:
            reserved[kind_root].update(
                PurePosixPath(path).stem for path, _ in _tree_blobs(root, commit, kind_root)
            )
    deletions = {
        PurePosixPath(path).stem
        for status, path in changes
        if status == "D" and path.startswith(offerings_root + "/")
    }
    for path, kind_root in additions:
        identifier = PurePosixPath(path).stem
        if identifier in reserved[kind_root] and not (
            kind_root == offerings_root and identifier in deletions
        ):
            diagnostics.append(
                Diagnostic(
                    "CHANGE_INVALID",
