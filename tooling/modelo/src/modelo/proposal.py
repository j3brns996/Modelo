...
OPERATIONS = {
    ...
    "revoke": "Withdraw approved access",
    "move": "Replace an offering",
    ...
    for kind, key in (
        ...
        for item in catalogue[key]:
            ...
            if kind == "evidence":
                ...
                observed = (
                    parse_rfc3339(item["observed_at"])
...
def render_fields(fields):
    ...
    lookup_kinds = {
        ...
        "source_identity": "offering",
        "destination_identity": "offering",
        "inference_service": "inference-service",
        "candidate_evidence": "evidence",
        "requested_outcome": "all",
    }
    for field in fields:
        name = field["name"]
        label = field["label"]
        ...
        if field["type"] == "dropdown":
            options = "".join(
                ...
                + '">'
                + escape(OPERATIONS.get(value, value.replace("-", " ").title()))
                + "</option>"
            ...
            length = (
                ...
                if name == "purpose"
                    ...
                    55000
                    if name in {"acceptance", "candidate_evidence"}
                    else (6500 if name == "subject_identities" else 2048)
                )
            )
            attrs += (
                f' maxlength="{length}" placeholder="{escape(field["placeholder"], quote=True)}"'
            )
            control = (
                f'<textarea {attrs}{required} rows="3"></textarea>'
        ...
        controls.append(
            ...
            + control
            + f'<p id="error-{name}" class="field-error" data-error="{name}"></p></div>'
            + lookup
            + "</div>"
        )
    return "\n".join(controls)
...
def render_gitlab_template(operation, fields, lookup_url):
    ...
    for field in relevant:
        ...
        if field.get("options") and field["name"] != "request_type":
            text += (
                "Choose: " + ", ".join("`" + value + "`" for value in field["options"]) + ".\n\n"
            )
    text += "## Your answers\n\n"
    for field in relevant:
